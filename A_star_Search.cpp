...
// Algorithm
template <size_t ROW, size_t COL>
vector<pair<int,int>> aStarSearch(const array<array<int, COL>, ROW>& grid,
...
int main()
{
    ...
    array<array<int, 25>, 40> grid; // на этой матрице тестить
    ...
    vector<pair<int,int>> a = aStarSearch(grid, src, dest);
    for(auto i:a){
